...
bool motionCaptureClientFramework::initConnection() {
    ...
    while (nTries--) {
        ...
        if ((nDataBytesReceived != 0) && (nDataBytesReceived != -1)) {
            ...
            auto server_info = (agile::sSender_Server *) (ptr + 4);
...
bool motionCaptureClientFramework::DecodeTimecode(
...
bool motionCaptureClientFramework::TimecodeStringify(
        unsigned int inTimecode,
        unsigned int inTimecodeSubframe,
        char *Buffer,
        size_t BufferSize) {
...
//         printf("Marker Set Count : %d\n", nMarkerSets);
        ...
        for (int i = 0; i < nMarkerSets; i++) {
            ...
            char szName[256];
            strcpy(szName, ptr);
            size_t nDataBytes = strlen(szName) + 1;
            ptr += nDataBytes;
//            printf("Model Name: %s\n", szName);
            //      output_packet_.model_name = szName;
...
//            printf("ori: [%3.2f,%3.2f,%3.2f,%3.2f]\n", qx, qy, qz, qw);
            ...
            if (major < 3) {
                ...
                size_t nBytes = nRigidMarkers * 3 * sizeof(float);
                auto markerData = (float *) malloc(nBytes);
                ...
                if (major >= 2) {
                    ...
                    auto markerSizes = (float *) malloc(nBytes);
            ...
            if (major >= 2) {
                ...
                memcpy(&fError, ptr, 4);
                ptr += 4;
//                printf("Mean marker error: %3.15f\n", fError);
...
//                printf("err:  [%3.2f]\n", residual);
        ...
        output_packet_.timestamp = (uint64_t) round((timestamp*1e9)/server_frequency);
        ...
        if (major >= 3) {
            ...
            output_packet_.mid_exposure_timestamp = (uint64_t) round((cameraMidExposureTimestamp*1e9)/server_frequency);
            output_packet_.camera_data_received_timestamp = (uint64_t) round((cameraDataReceivedTimestamp*1e9)/server_frequency);
            output_packet_.transmit_timestamp = (uint64_t) round((transmitTimestamp*1e9)/server_frequency);
        }
...
bool motionCaptureClientFramework::IPAddress_StringToAddr(
